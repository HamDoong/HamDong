...
from apps.expenses.infrastructure.auth_errors import JWTPublicKeyUnavailable
from apps.expenses.infrastructure.exception_handlers import api_exception_handler
from apps.expenses.infrastructure.jwks_client import JWKSClientError
...
def make_request(token: str | None = None):
    ...
    if token is None:
        return factory.get("/api/v1/expenses/")
    return factory.get("/api/v1/expenses/", HTTP_AUTHORIZATION=f"Bearer {token}")
