...
from apps.settlements.infrastructure.auth_errors import JWTPublicKeyUnavailable
from apps.settlements.infrastructure.exception_handlers import api_exception_handler
from apps.settlements.infrastructure.jwks_client import JWKSClientError
...
def make_request(token: str | None = None):
    ...
    if token is None:
        return factory.get("/api/v1/settlements/")
    return factory.get("/api/v1/settlements/", HTTP_AUTHORIZATION=f"Bearer {token}")
