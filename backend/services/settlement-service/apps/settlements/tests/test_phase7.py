...
def create_member(group_id, user_id=None, status=GroupMemberStatusChoices.ACTIVE, phone_number="+989121234567", role="MEMBER"):
...
class DebtServiceTests(TestCase):
    def setUp(self):
        ...
        self.debt_service = DebtService(publisher=self.publisher, balance_service=self.balance_service)
    ...
    def _expense_payload(self, expense_id=None, participants=None, version=1, total_amount_minor=3000):
        expense_id = expense_id or uuid.uuid4()
        participants = participants or [
            {"user_id": str(self.payer.user_id), "total_share_minor": 1000, "base_share_minor": 1000, "tax_share_minor": 0, "service_fee_share_minor": 0},
            {"user_id": str(self.member_a.user_id), "total_share_minor": 1000, "base_share_minor": 1000, "tax_share_minor": 0, "service_fee_share_minor": 0},
            {"user_id": str(self.member_b.user_id), "total_share_minor": 1000, "base_share_minor": 1000, "tax_share_minor": 0, "service_fee_share_minor": 0},
        ]
        return {
    ...
    def test_expense_created_creates_projection_and_participants_and_debt_entries(self):
        ...
        self.assertEqual(ExpenseParticipantProjectionRepository.list_for_expense(payload["expense_id"]).count(), 3)
        self.assertEqual(len(entries), 2)
        self.assertFalse(any(entry.debtor_user_id == self.payer.user_id and entry.creditor_user_id == self.payer.user_id for entry in entries))
        self.balance_service.recalculate_group.assert_called_once_with(expense.group_id, currency="IRR")
        published_types = [call.args[0] for call in self.publisher.publish.call_args_list]
    ...
    def test_expense_updated_reverses_previous_entries_and_creates_new_entries(self):
        ...
        updated_payload = self._expense_payload(
            ...
            version=2,
            participants=[
                {"user_id": str(self.payer.user_id), "total_share_minor": 1500, "base_share_minor": 1500, "tax_share_minor": 0, "service_fee_share_minor": 0},
                {"user_id": str(self.member_a.user_id), "total_share_minor": 1500, "base_share_minor": 1500, "tax_share_minor": 0, "service_fee_share_minor": 0},
            ],
        ...
        reversed_count = DebtLedgerEntry.objects.filter(source_expense_id=expense.expense_id, status=DebtLedgerStatusChoices.REVERSED).count()
        active_count = DebtLedgerEntry.objects.filter(source_expense_id=expense.expense_id, status=DebtLedgerStatusChoices.ACTIVE).count()
    ...
    def test_expense_deleted_reverses_active_entries_and_recalculates(self):
        ...
        self.balance_service.reset_mock()
        self.publisher.reset_mock()
        self.debt_service.handle_expense_deleted({"expense_id": str(expense.expense_id)})

        self.assertEqual(DebtLedgerEntry.objects.filter(source_expense_id=expense.expense_id, status=DebtLedgerStatusChoices.ACTIVE).count(), 0)
        self.assertGreaterEqual(DebtLedgerEntry.objects.filter(source_expense_id=expense.expense_id, status=DebtLedgerStatusChoices.REVERSED).count(), 1)
        self.balance_service.recalculate_group.assert_called_once_with(expense.group_id, currency="IRR")
        published_types = [call.args[0] for call in self.publisher.publish.call_args_list]
        self.assertIn("DebtLedgerUpdated", published_types)
        self.assertIn("BalanceRecalculated", published_types)
...
class SettlementApiTests(TestCase):
    ...
    def test_group_balances_endpoint_returns_payload(self):
        ...
        response = self.client.get(reverse("group_balances", kwargs={"group_id": self.group.group_id}))
    ...
    def test_group_balances_denies_left_member(self):
        ...
        create_member(self.group.group_id, user_id=left_user_id, status=GroupMemberStatusChoices.LEFT)
        self.client.force_authenticate(user=SimpleNamespace(sub=left_user_id, is_authenticated=True))
        response = self.client.get(reverse("group_balances", kwargs={"group_id": self.group.group_id}))
        self.assertEqual(response.status_code, 403)

    def test_my_balance_endpoint_returns_current_user_balance(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.get(reverse("my_balance", kwargs={"group_id": self.group.group_id}))
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json()["user_id"], str(self.user.sub))

    def test_group_debts_endpoint_returns_only_active_debts(self):
        self.client.force_authenticate(user=self.user)
        response = self.client.get(reverse("group_debts", kwargs={"group_id": self.group.group_id}))
        self.assertEqual(response.status_code, 200)
    ...
    def test_create_manual_settlement_returns_201(self):
        self.client.force_authenticate(user=self.user)
        with patch("apps.settlements.application.settlement_service.RabbitMQPublisher.publish", return_value=True):
...
class SettlementWorkflowServiceTests(TestCase):
    def setUp(self):
        ...
        self.balance_service = Mock()
        self.debt_service = DebtService(publisher=self.publisher, balance_service=self.balance_service)
        self.service = SettlementService(publisher=self.publisher, debt_service=self.debt_service, balance_service=self.balance_service)
        self.group = create_group()
        self.payer = create_member(self.group.group_id)
    ...
    def test_create_settlement_success_and_event_published(self):
        settlement = self.service.create_manual_settlement(
            self.group.group_id,
            self.payer.user_id,
            {"receiver_user_id": str(self.receiver.user_id), "amount_minor": 1000, "currency": "IRR", "description": "Taxi"},
        )
        self.assertEqual(settlement.status, ManualSettlementStatusChoices.PENDING_CONFIRMATION)
        self.assertEqual(settlement.created_by_user_id, self.payer.user_id)
        self.publisher.publish.assert_any_call("SettlementCreated", ANY, "settlement.created")
    ...
    def test_create_settlement_same_payer_receiver_fails(self):
        with self.assertRaises(InvalidSettlementParticipantError):
            self.service.create_manual_settlement(
                self.group.group_id,
                self.payer.user_id,
                {"receiver_user_id": str(self.payer.user_id), "amount_minor": 1000, "currency": "IRR"},
            )
    ...
    def test_create_settlement_non_member_receiver_fails(self):
        with self.assertRaises(NotGroupMemberError):
            self.service.create_manual_settlement(
                self.group.group_id,
                self.payer.user_id,
                {"receiver_user_id": str(uuid.uuid4()), "amount_minor": 1000, "currency": "IRR"},
            )
    ...
    def test_create_settlement_invalid_amount_fails(self):
        with self.assertRaises(InvalidSettlementAmountError):
            self.service.create_manual_settlement(
                self.group.group_id,
                self.payer.user_id,
                {"receiver_user_id": str(self.receiver.user_id), "amount_minor": 0, "currency": "IRR"},
            )
...
class SettlementConsumerTests(TestCase):
    ...
    def test_duplicate_event_id_is_ignored(self):
        ...
        with patch("apps.settlements.infrastructure.rabbitmq_consumer.ProcessedEventRepository.was_processed", return_value=True), \
    ...
    def test_processed_event_prevents_double_debt_creation(self):
        ...
        with patch.object(self.consumer.expense_events, "handle", return_value=None) as handle_mock:
    ...
    def test_identity_payload_is_idempotent(self):
        ...
        with patch("apps.settlements.infrastructure.rabbitmq_consumer.ProcessedEventRepository.was_processed", return_value=False), \
            patch("apps.settlements.infrastructure.rabbitmq_consumer.UserProjectionRepository.upsert_from_event", return_value=SimpleNamespace()), \
            patch("apps.settlements.infrastructure.rabbitmq_consumer.ProcessedEventRepository.mark_processed") as mark_processed:
    ...
    def test_group_created_and_member_joined_and_removed_updates_projection(self):
        ...
        member = GroupMemberProjection.objects.get(group_id=group_id, user_id=member_user_id)
