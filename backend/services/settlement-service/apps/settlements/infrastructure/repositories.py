...
class GroupProjectionRepository:
	...
	def upsert_from_event(**data):
		...
		defaults = {
			...
			"status": data.get("status") or (current.status if current else GroupStatusChoices.ACTIVE),
			"created_by_user_id": normalize_uuid(data.get("created_by_user_id") or data.get("created_by") or (current.created_by_user_id if current else group_id)),
			"member_count": data.get("member_count") if data.get("member_count") is not None else (current.member_count if current else 0),
		}
	...
	def refresh_member_count(group_id):
		...
		if not group:
			return None
		group.member_count = GroupMemberProjection.objects.filter(group_id=group.group_id, status="ACTIVE").count()
...
class GroupMemberProjectionRepository:
	...
	def upsert_joined(**data):
		...
		phone_number = data.get("phone_number")
		if not phone_number:
			fallback_display, phone_number = GroupMemberProjectionRepository._display_snapshot(user_id)
	...
	def get_active_member(group_id, user_id):
		return GroupMemberProjection.objects.filter(group_id=normalize_uuid(group_id), user_id=normalize_uuid(user_id), status="ACTIVE").first()

	@staticmethod
	def get(group_id, user_id):
		return GroupMemberProjection.objects.filter(group_id=normalize_uuid(group_id), user_id=normalize_uuid(user_id)).first()

	@staticmethod
	def list_active_members(group_id):
		return GroupMemberProjection.objects.filter(group_id=normalize_uuid(group_id), status="ACTIVE")
...
class ExpenseProjectionRepository:
	...
	def upsert_from_event(**data):
		...
		group_id = normalize_uuid(data.get("group_id") or (current.group_id if current else None))
		created_by_user_id = normalize_uuid(data.get("created_by_user_id") or (current.created_by_user_id if current else group_id))
		payer_user_id = normalize_uuid(data.get("payer_user_id") or (current.payer_user_id if current else created_by_user_id))
		defaults = {
			"group_id": group_id,
			"created_by_user_id": created_by_user_id,
			"payer_user_id": payer_user_id,
			"currency": data.get("currency") or (current.currency if current else CurrencyChoices.IRR),
			"base_amount_minor": int(data.get("base_amount_minor") or (current.base_amount_minor if current else 0)),
			"tax_amount_minor": int(data.get("tax_amount_minor") or (current.tax_amount_minor if current else 0)),
			"service_fee_amount_minor": int(data.get("service_fee_amount_minor") or (current.service_fee_amount_minor if current else 0)),
			"total_amount_minor": int(data.get("total_amount_minor") or (current.total_amount_minor if current else 0)),
			"status": data.get("status") or (current.status if current else ExpenseStatusChoices.ACTIVE),
			"expense_version": int(data.get("expense_version") or data.get("new_version") or (current.expense_version if current else 1)),
			"expense_date": data.get("expense_date") or (current.expense_date if current else timezone.now()),
		}
	...
	@staticmethod
	def get(expense_id):
		return ExpenseProjection.objects.filter(expense_id=normalize_uuid(expense_id)).first()
	...
	def mark_deleted(expense_id):
		...
		expense.save(update_fields=["status", "deleted_at", "expense_version", "updated_at"])
		return expense

	@staticmethod
	def list_by_group(group_id):
		return ExpenseProjection.objects.filter(group_id=normalize_uuid(group_id)).order_by("-created_at")


class ExpenseParticipantProjectionRepository:
	@staticmethod
	def replace_for_expense(expense: ExpenseProjection, participants):
		ExpenseParticipantProjection.objects.filter(expense_id=expense.expense_id).delete()
		rows = []
	...
	def list_for_expense(expense_id):
		return ExpenseParticipantProjection.objects.filter(expense_id=normalize_uuid(expense_id)).order_by("created_at")

	@staticmethod
	def delete_for_expense(expense_id):
		ExpenseParticipantProjection.objects.filter(expense_id=normalize_uuid(expense_id)).delete()
...
class DebtLedgerRepository:
	...
	def reverse_active_for_expense(expense_id):
		now = timezone.now()
		entries = DebtLedgerEntry.objects.filter(source_expense_id=normalize_uuid(expense_id), status=DebtLedgerStatusChoices.ACTIVE)
		entries.update(status=DebtLedgerStatusChoices.REVERSED, reversed_at=now)
		return list(entries)

	@staticmethod
	def active_for_expense(expense_id):
		return DebtLedgerEntry.objects.filter(source_expense_id=normalize_uuid(expense_id), status=DebtLedgerStatusChoices.ACTIVE)

	@staticmethod
	def active_by_group(group_id):
		return DebtLedgerEntry.objects.filter(group_id=normalize_uuid(group_id), status=DebtLedgerStatusChoices.ACTIVE).order_by("created_at")

	@staticmethod
	def all_by_group(group_id):
		return DebtLedgerEntry.objects.filter(group_id=normalize_uuid(group_id)).order_by("created_at")
...
class GroupBalanceSnapshotRepository:
	...
	def upsert_snapshot(group_id, user_id, currency, totals):
		defaults = {
			...
			"total_settled_received_minor": totals.get("total_settled_received_minor", 0),
	...
	def list_by_group(group_id, currency=None):
		qs = GroupBalanceSnapshot.objects.filter(group_id=normalize_uuid(group_id)).order_by("user_id")
		if currency:
	...
	def get(group_id, user_id, currency="IRR"):
		return GroupBalanceSnapshot.objects.filter(group_id=normalize_uuid(group_id), user_id=normalize_uuid(user_id), currency=currency).first()

	@staticmethod
	def delete_missing(group_id, user_ids, currency="IRR"):
		GroupBalanceSnapshot.objects.filter(group_id=normalize_uuid(group_id), currency=currency).exclude(user_id__in=[normalize_uuid(user_id) for user_id in user_ids]).delete()
...
class ManualSettlementRepository:
	...
	def create_pending(**data):
		return ManualSettlement.objects.create(
			...
			created_by_user_id=normalize_uuid(data.get("created_by_user_id") or data.get("payer_user_id")),
		)
	...
	def list_by_group(group_id, filters=None):
		qs = ManualSettlement.objects.filter(group_id=normalize_uuid(group_id)).order_by("-created_at")
		filters = filters or {}
	...
	def confirm(settlement: ManualSettlement, confirmed_by_user_id):
		...
		settlement.save(update_fields=["status", "confirmed_by_user_id", "confirmed_at", "updated_at"])
		return settlement
	...
	def reject(settlement: ManualSettlement, rejected_by_user_id):
		...
		settlement.save(update_fields=["status", "rejected_by_user_id", "rejected_at", "updated_at"])
		return settlement
	...
	def cancel(settlement: ManualSettlement, cancelled_by_user_id):
		...
		settlement.save(update_fields=["status", "cancelled_by_user_id", "cancelled_at", "updated_at"])
		return settlement
...
class ProcessedEventRepository:
	...
	def mark_processed(event_id, event_type, source_service):
		...
		obj, created = ProcessedEvent.objects.get_or_create(
			event_id=event_id,
			defaults={"event_type": event_type, "source_service": source_service, "processed_at": timezone.now()},
		)
		return obj, created
