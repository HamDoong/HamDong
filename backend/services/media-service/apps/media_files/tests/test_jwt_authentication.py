...
from apps.media_files.infrastructure.exception_handlers import api_exception_handler
from apps.media_files.infrastructure.jwks_client import JWKSClientError
...
def make_request(token: str | None = None):
    ...
    if token is None:
        return factory.get("/api/v1/media/")
    return factory.get("/api/v1/media/", HTTP_AUTHORIZATION=f"Bearer {token}")
