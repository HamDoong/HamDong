...
from apps.identity.application.use_cases import (
    ...
    GetPublicUserByIdUseCase,
    LogoutUseCase,
    PasswordLoginUseCase,
    PasswordResetUseCase,
    RefreshTokenUseCase,
    RequestOtpUseCase,
    SearchUsersByArtNameUseCase,
    SessionListUseCase,
    SetPasswordUseCase,
...
ErrorDetailSerializer = inline_serializer(
    name="IdentityErrorDetailSerializer",
    fields={
...
def _error_response(
    code: str, message: str, http_status: int, details=None
) -> Response:
...
class RequestOtpView(APIView):
    ...
    @extend_schema(
        ...
        description=(
            "Requests an OTP for LOGIN or SIGNUP. "
            "Rejected requests do not create or store an OTP and do not enqueue an email event."
        ),
        request=RequestOtpSerializer,
        responses={
            200: OpenApiResponse(description="OTP requested"),
            400: OpenApiResponse(response=ErrorResponseSerializer, description="Validation or purpose error."),
            403: OpenApiResponse(response=ErrorResponseSerializer, description="Account is deactivated."),
            404: OpenApiResponse(response=ErrorResponseSerializer, description="Email not registered for LOGIN."),
            409: OpenApiResponse(response=ErrorResponseSerializer, description="Signup/login state conflict."),
            429: OpenApiResponse(response=ErrorResponseSerializer, description="Rate limited or cooldown active."),
        },
        examples=[
            OpenApiExample(
                "Request OTP success",
    ...
    def post(self, request):
        ...
        if not success:
            ...
            if error_code == "SIGNUP_NOT_COMPLETED":
                return _error_response(
                    "SIGNUP_NOT_COMPLETED",
                    "Signup is not completed for this email.",
...
class VerifyOtpView(APIView):
    ...
    @extend_schema(
        ...
        responses={
            ...
            400: OpenApiResponse(response=ErrorResponseSerializer, description="Validation or OTP error."),
            403: OpenApiResponse(response=ErrorResponseSerializer, description="Account is deactivated."),
            404: OpenApiResponse(response=ErrorResponseSerializer, description="User not found."),
            409: OpenApiResponse(response=ErrorResponseSerializer, description="Signup/login state conflict."),
            429: OpenApiResponse(response=ErrorResponseSerializer, description="Maximum attempts exceeded."),
        },
    )
    def post(self, request):
...
class ForgotPasswordRequestView(APIView):
    ...
    def post(self, request):
        ...
        success, error_code, debug_otp = ForgotPasswordRequestUseCase().execute(
            serializer.validated_data["email"]
        )
        if not success:
            if error_code == "OTP_RATE_LIMITED":
            ...
            return _error_response(
                "INVALID_REQUEST", "Invalid request data.", status.HTTP_400_BAD_REQUEST
            )
    ...
    def post(self, request):
        ...
        if not success:
            mapping = {
                ...
                "OTP_ALREADY_USED": (
                    "OTP_ALREADY_USED",
                    "The OTP code has already been used.",
                    status.HTTP_400_BAD_REQUEST,
                ),
                "OTP_MAX_ATTEMPTS_EXCEEDED": (
                    "OTP_MAX_ATTEMPTS_EXCEEDED",
...
class SessionsView(APIView):
    ...
    def get(self, request):
        ...
        results = SessionListUseCase().execute(
            user=user, current_jti=getattr(request.user, "token_jti", None)
        )
    ...
    def delete(self, request):
        ...
        revoked_count = DeleteAllSessionsUseCase().execute(
            user=user, current_jti=getattr(request.user, "token_jti", None)
        )
        return Response(
            {
...
class SessionDetailView(APIView):
    ...
    def delete(self, request, session_id):
        ...
        if not success:
            return _error_response(
                "SESSION_NOT_FOUND", "Session not found.", status.HTTP_404_NOT_FOUND
            )
        ...
        if not success:
            mapping = {
                "PASSWORD_ALREADY_SET": (
                    "PASSWORD_ALREADY_SET",
                    "Password is already set. Use change password endpoint.",
                    status.HTTP_409_CONFLICT,
                ),
                "PASSWORD_CONFIRMATION_MISMATCH": (
                    "PASSWORD_CONFIRMATION_MISMATCH",
...
class UserSearchView(APIView):
    ...
    @extend_schema(
        tags=["Users"],
        summary="Search active users by art name",
        description=(
            "Searches active, non-deleted users by art_name only. "
            "Returns public-safe search results for invite and group creation flows."
        ),
        parameters=[
            OpenApiParameter(
                name="art_name",
                ...
                required=True,
                description="Case-insensitive art_name query (2-50 characters).",
            ),
            OpenApiParameter(
                name="limit",
                type=int,
            ...
            OpenApiParameter(
                ...
                required=False,
                default=True,
                description="Exclude the authenticated user from results.",
            ),
        ],
        responses={
        ...
        examples=[
            OpenApiExample(
                ...
                value={
                    "items": [
                        {
                            "user_id": "f8c1f9ef-1111-4444-9999-abc123abc123",
                            "art_name": "amir_art",
                            "avatar_url": None,
                        }
                    ],
    ...
    def get(self, request):
        ...
        if not serializer.is_valid():
            if "art_name" in serializer.errors:
                art_name = request.query_params.get("art_name")
                if art_name is None:
                    return _error_response(
                        "ART_NAME_QUERY_REQUIRED",
                        "art_name query parameter is required.",
                        status.HTTP_400_BAD_REQUEST,
                    )
                error_code = serializer.errors["art_name"][0]
                if error_code == "ART_NAME_QUERY_TOO_SHORT":
                    return _error_response(
                        "ART_NAME_QUERY_TOO_SHORT",
                        "art_name must be at least 2 characters.",
                        status.HTTP_400_BAD_REQUEST,
                        serializer.errors,
                    )
                if error_code == "ART_NAME_QUERY_TOO_LONG":
                    return _error_response(
                        "ART_NAME_QUERY_TOO_LONG",
                        "art_name must be at most 50 characters.",
                        status.HTTP_400_BAD_REQUEST,
                        serializer.errors,
                    )
                return _error_response(
            ...
            if "exclude_me" in serializer.errors:
                return _error_response(
                    "INVALID_REQUEST",
                    "Invalid request data.",
                    status.HTTP_400_BAD_REQUEST,
        ...
        query = serializer.validated_data["art_name"]
        limit = serializer.validated_data["limit"]
        exclude_me = serializer.validated_data["exclude_me"]
        items = SearchUsersByArtNameUseCase().execute(
            query=query,
            limit=limit,
            exclude_user_id=request.user.id if exclude_me else None,
        )
        response_data = {"items": items, "count": len(items), "query": query}
        return Response(
            UserSearchResponseSerializer(response_data).data, status=status.HTTP_200_OK
        )


class PublicUserLookupView(APIView):
    authentication_classes = [JWTAuthentication]
    permission_classes = [IsAuthenticated]

    @extend_schema(
        tags=["Users"],
        summary="Get a public-safe user preview by id",
        description=(
            "Returns a public-safe preview for an active, non-deleted user. "
            "This endpoint does not create invitations or group memberships."
        ),
        responses={
            200: PublicUserSerializer,
            400: OpenApiResponse(
                response=ErrorResponseSerializer, description="Invalid user id."
            ),
            401: OpenApiResponse(
                response=ErrorResponseSerializer, description="Authentication required."
            ),
    ...
    def get(self, request, user_id):
        ...
        user = GetPublicUserByIdUseCase().execute(user_id=normalized_user_id)
        if not user:
            return _error_response(
                "USER_NOT_FOUND", "User was not found.", status.HTTP_404_NOT_FOUND
            )
    ...
    def patch(self, request):
        ...
        if not serializer.is_valid():
            ...
            for field_name, (code, message) in error_mappings.items():
                if field_name in serializer.errors:
                    return _error_response(
                        code, message, status.HTTP_400_BAD_REQUEST, serializer.errors
                    )
            return _error_response(
                "INVALID_REQUEST",
                "Invalid request data.",
        ...
        try:
            ...
            if str(exc) == "ART_NAME_ALREADY_EXISTS":
                return _error_response(
                    "ART_NAME_ALREADY_EXISTS",
                    "This art name is already taken.",
                    status.HTTP_409_CONFLICT,
                )
...
class MeBankCardsView(APIView):
    ...
    @extend_schema(
        ...
        parameters=[
            OpenApiParameter(name="include_inactive", required=False, type=bool)
        ],
    ...
    def post(self, request):
        ...
        try:
            card, created = BankCardService().create_card(
                user, **serializer.validated_data
            )
        except BankCardError as exc:
            http_status = (
                status.HTTP_409_CONFLICT
                if exc.code in {"CARD_ALREADY_EXISTS", "BANK_CARD_LIMIT_EXCEEDED"}
                else status.HTTP_400_BAD_REQUEST
            )
            return _error_response(exc.code, exc.message, http_status, exc.fields)
        return Response(
            serialize_bank_card(card),
...
class MeBankCardDetailView(APIView):
    ...
    @extend_schema(
        ...
        request=UpdateUserBankCardSerializer,
        responses={200: UserBankCardSerializer},
    )
    def patch(self, request, card_id):
        ...
        try:
            card = BankCardService().update_card(
                user, card_id, serializer.validated_data
            )
        except BankCardError as exc:
            http_status = (
                status.HTTP_404_NOT_FOUND
    ...
    @extend_schema(
        ...
        responses={204: OpenApiResponse(description="Bank card deactivated")},
    )
...
class MeBankCardsBulkView(APIView):
    ...
    @extend_schema(
        tags=["Users"],
        summary="Bulk save bank cards",
        request=BulkUserBankCardSaveSerializer,
        responses={200: BulkUserBankCardSaveResponseSerializer},
    )
    def put(self, request):
        user = UserRepository.get_by_id(request.user.id)
        if not user:
        ...
        try:
            ...
            http_status = (
                status.HTTP_409_CONFLICT
                if exc.code in {"BANK_CARD_LIMIT_EXCEEDED"}
                else status.HTTP_400_BAD_REQUEST
            )
            if exc.code == "NOT_FOUND":
...
class DeactivateAccountView(APIView):
    ...
    @extend_schema(
        ...
        responses={200: DeactivateAccountResponseSerializer},
    )
    def post(self, request):
        user = UserRepository.get_by_id(request.user.id)
        if not user:
        ...
        try:
            ...
            return _error_response(
                exc.code, exc.message, status.HTTP_400_BAD_REQUEST, exc.fields
            )
...
class InternalPaymentContextBankCardsView(APIView):
    ...
    def _is_authorized(self, request) -> bool:
        expected = str(
            getattr(settings, "INTERNAL_SERVICE_TOKEN", "hamdong-internal-token")
        )
        provided = request.headers.get("X-Internal-Service-Token") or request.META.get(
            "HTTP_X_INTERNAL_SERVICE_TOKEN"
        )
    ...
    def post(self, request):
        if not self._is_authorized(request):
            return _error_response(
                "FORBIDDEN", "Internal authorization failed.", status.HTTP_403_FORBIDDEN
            )
